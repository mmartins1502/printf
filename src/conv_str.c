...
/*   Updated: 2017/05/07 22:10:33 by mmartins         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			conv_str(t_br *br, va_list ap, t_flag flag)
{
	...
	size = ft_strlen(str);
	if (flag.prec >= 0)
		size = size < flag.prec ? ft_strlen(str) : flag.prec;
	else
		size = ft_strlen(str);
	c = flag.zero == 1 ? '0' : ' ';
	if (flag.width > (unsigned int)size && flag.minus == 0)
		ft_width_str(c, flag.width - size, br);
	while (str[++i] && i < size)
		ft_br(str[i], br);
	if (flag.width > (unsigned int)size && flag.minus == 1)
		ft_width_str(c, flag.width - size, br);
	return (1);
}
